...
<body class="cnt-home">
	...
	<form method="post" action="CancelOrderServlet.credu">
			...
			<table id="goods_list" class="table">
				<thead>
					<tr>
						...
						<th width="50%" style="text-align: center;">상 품</th>
						<th width="30%" style="text-align: center;">주문수량 및 색상</th>
						<th width="10%" style="text-align: center;">가 격</th>
						<th width="10%" style="text-align: center;">상 태</th>
					</tr>
				</thead>
				<tbody>
					...
					<%	for(int i = 0; i < bean.size() ; i++){ %>
						<tr>
							<td><input type="checkbox" id="check_list<%=i%>" name="check_list" onchange="check_sum()" data-goods_price="<%=bean.get(i).getBox_goods_price() %>" data-c_index="<%=i%>" value="<%=i%>"></td>
							<td><img src="<%=image[i] %>" width="150px" height="200px"/>상품명 Text</td>
							<td>
								<label>빨강 : <%=bean.get(i).getGoods_color_num_1() %></label>
								<label>검정 : <%=bean.get(i).getGoods_color_num_2() %></label></br>
								<label>노랑 : <%=bean.get(i).getGoods_color_num_3() %></label>
								<label>파랑 : <%=bean.get(i).getGoods_color_num_4() %></label>															
							</td>
							<td>
								<label><%=bean.get(i).getBox_goods_price()%></label>
							</td>
							<td id="order<%=i%>"> 
								<a href ="#" class="btn btn-warning" data-box_num="<%=bean.get(i).getBox_num() %>">주문취소</a>
							</td>
							<td><input id="box_num<%=i %>" type="hidden" name="box_num" value="<%=bean.get(i).getBox_num()%>"/></td>
							<td><input type="hidden" name="order_num" value="<%=bean.get(i).getGoods_num()%>"/></td>
							<td><input type="hidden" name="delivery_idx" value="<%=bean.get(i).getDelivery_idx() %>"/></td>
						</tr>
					<%	} %>
